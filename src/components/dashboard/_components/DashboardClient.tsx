...
export function DashboardClient() {
  ...
  const summary = insights?.summary ?? { developerCount: 0, avgComposite: 0, avgJira: 0, avgGitlab: 0 };
  ...
  function renderWidget(widget: WidgetConfig): ReactNode {
    ...
    switch (widget.type) {
      case 'trend-line': {
        const rangeMonths = { '3m': 3, '6m': 6, '12m': 12 }[(widget.props?.range as string) ?? ''] ?? 0;
        const visibleTrend = rangeMonths > 0 ? trendData.slice(-rangeMonths) : trendData;
        return (
          <TrendOverview
            trendData={visibleTrend}
      ...
      case 'ranking-table': {
        const sortPropMap: Record<string, 'compositeScore' | 'jiraScore' | 'gitlabScore'> = {
          composite: 'compositeScore',
          jira: 'jiraScore',
          gitlab: 'gitlabScore',
        };
        const defaultSortKey = sortPropMap[(widget.props?.sort as string) ?? ''] ?? 'compositeScore';
        const limitProp = widget.props?.limit as number | undefined;
        const displayData = limitProp && limitProp > 0 ? rankingData.slice(0, limitProp) : rankingData;
        return <DeveloperRankingTable data={displayData} defaultSortKey={defaultSortKey} />;
      }
      case 'score-gauge': {
        const metric = widget.props?.metric as string | undefined;
        ...
        if (metric === 'composite') {
          return (
            <ScoreGauge
              score={summary.avgComposite}
              label="팀 종합점수"
              helperText={`${summary.developerCount}명 평균`}
              size="sm"
              className="h-full justify-center"
            />
          );
