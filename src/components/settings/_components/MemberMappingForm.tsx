...
interface MemberMappingSectionsProps {
 ...
 */
export function MemberMappingSections({ projectsVersion = 0 }: MemberMappingSectionsProps) {
  ...
  const loadProjectsOnly = useCallback(async () => {
    try {
      ...
      const json = (await res.json()) as ApiResponse<Array<{ id: string; name: string; type: 'jira' | 'gitlab' }>>;
      if (json.success && json.data) {
        setProjects(json.data.map((project) => ({ id: project.id, name: project.name, type: project.type })));
      }
    } catch (error) {
      console.error('프로젝트 목록 재조회 실패:', error);
    }
  }, []);
  ...
  useEffect(() => {
    if (projectsVersion > 0) {
      void loadProjectsOnly();
    }
  }, [projectsVersion, loadProjectsOnly]);
