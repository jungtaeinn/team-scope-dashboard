...
import { ChevronDown, KeyRound, Loader2, MailPlus, Save, Trash2 } from 'lucide-react';
...
export function WorkspaceAccessForm() {
  ...
  const [resetConfirmTarget, setResetConfirmTarget] = useState<{ memberId: string; memberName: string } | null>(null);
  ...
  const handleResetPassword = useCallback((memberId: string, memberName: string) => {
    setResetConfirmTarget({ memberId, memberName });
  }, []);

  const handleResetPasswordConfirm = useCallback(async () => {
    if (!resetConfirmTarget) return;
    const { memberId, memberName } = resetConfirmTarget;

    setResetPendingId(memberId);
    setResetConfirmTarget(null);
    setStatusMessage(null);
    setErrorMessage(null);

    try {
      const response = await fetch('/api/workspace/users/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ memberId }),
      });
      const json = (await response.json().catch(() => null)) as { success: boolean; error: string | null } | null;

      if (!response.ok || !json?.success) {
        throw new Error(json?.error ?? '비밀번호 초기화에 실패했습니다.');
      }

      setStatusMessage(`${memberName}의 비밀번호를 qwer1234로 초기화했습니다. 다음 로그인 시 변경을 안내받습니다.`);
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : '비밀번호 초기화에 실패했습니다.');
    } finally {
  ...
  return (
    <section className="space-y-4">
                                ...
                                <td className="whitespace-nowrap px-4 py-3 text-sm">
                                  <div className="flex items-center gap-1">
                                    ...
                                    {!member.isCurrentUser ? (
                                      <button
                                        type="button"
                                        onClick={() => void handleResetPassword(member.id, member.name)}
                                        disabled={resetPendingId === member.id}
                                        className="rounded p-1 text-amber-500 hover:bg-amber-50 disabled:cursor-not-allowed disabled:opacity-40 dark:hover:bg-amber-900/20"
                                        title="비밀번호 초기화 (qwer1234)"
                                        aria-label={`${member.name} 비밀번호 초기화`}
                                      >
                                        {resetPendingId === member.id ? (
                                          <Loader2 className="h-4 w-4 animate-spin" />
                                        ) : (
      ...
      {/* ── 비밀번호 초기화 확인 모달 ── */}
      {resetConfirmTarget ? (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/45 px-4" onClick={() => setResetConfirmTarget(null)}>
          <div
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-base font-semibold text-foreground">비밀번호 초기화</p>
                <p className="mt-1 text-sm text-muted-foreground">
                  <span className="font-medium text-foreground">{resetConfirmTarget.memberName}</span>의 비밀번호를 초기화합니다.
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-800/40 dark:bg-amber-900/20">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-300">초기화 후 변경사항</p>
              <ul className="mt-1.5 space-y-1 text-sm text-amber-700 dark:text-amber-400">
                <li>• 비밀번호가 <span className="font-mono font-semibold">qwer1234</span>로 변경됩니다.</li>
                <li>• 기존 로그인 세션이 즉시 만료됩니다.</li>
                <li>• 다음 로그인 시 비밀번호 변경을 안내받습니다.</li>
              </ul>
            </div>
