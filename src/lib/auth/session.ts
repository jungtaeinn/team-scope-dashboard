...
import { ensureUserPersonalWorkspace } from '@/lib/workspaces/personal-workspace.server';
...
async function resolveWorkspaceContextFromSession(session: Awaited<ReturnType<typeof auth.api.getSession>>) {
  ...
  const [existingMembership, dbUser] = await Promise.all([
  ...
  const { organization, member } = await ensureUserPersonalWorkspace(
    session.user,
    normalizeRole(existingMembership?.role ?? 'owner'),
  );
