...
export async function POST(request: Request) {
  try {
    ...
    // Determine which data sources are needed across all requested sheets
    const needsJira = sheetKeys.some((k) => k === 'developerDetail' || k === 'jiraIssues');
    const needsGitlab = sheetKeys.some((k) => k === 'developerDetail' || k === 'gitlabMrs');

    // Batch-fetch all issue/MR data in parallel — eliminates the previous N+1 pattern
    const [allJiraIssues, allGitlabMRs]: [any[], any[]] = await Promise.all([
      needsJira
    ...
    // Group by developer for O(1) per-developer lookup
    const issuesByDev = new Map<string, any[]>();
    for (const issue of allJiraIssues) {
      if (!issue.assigneeId) continue;
      if (!issuesByDev.has(issue.assigneeId)) issuesByDev.set(issue.assigneeId, []);
      issuesByDev.get(issue.assigneeId)!.push(issue);
    }

    const mrsByDev = new Map<string, any[]>();
    for (const mr of allGitlabMRs) {
      if (!mr.authorId) continue;
      if (!mrsByDev.has(mr.authorId)) mrsByDev.set(mr.authorId, []);
      mrsByDev.get(mr.authorId)!.push(mr);
    }
    ...
    if (sheetKeys.includes('developerDetail')) {
      for (const dev of developers as any[]) {
        const score = scoreByDeveloper.get(dev.id);
        const breakdown = score ? safeParseBreakdown(score.breakdown) : null;
        const jiraIssues = issuesByDev.get(dev.id) ?? [];
        const gitlabMRs = mrsByDev.get(dev.id) ?? [];
    ...
    if (sheetKeys.includes('jiraIssues')) {
      sheets.push({
        name: 'Jira 이슈',
        worksheet: buildJiraSheet(allJiraIssues.map(toJiraParsed) as unknown as Record<string, unknown>[]),
      });
    }
    ...
    if (sheetKeys.includes('gitlabMrs')) {
      sheets.push({
        name: 'GitLab MR',
        worksheet: buildGitlabSheet(allGitlabMRs.map(toGitlabParsed) as unknown as Record<string, unknown>[]),
      });
    }
