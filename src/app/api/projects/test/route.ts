...
async function testGitlabConnection(body: TestConnectionBody) {
  ...
  try {
    ...
    let projectCheck = null;
    // projectKey를 직접 입력했거나, URL 경로에서 그룹/프로젝트 경로를 추출할 수 있으면 감지 시도
    const effectiveKey =
      body.projectKey?.trim() ||
      getGitlabProjectPathFromUrl(body.baseUrl) ||
      getGitlabGroupPathFromUrl(body.baseUrl) ||
      null;
    ...
    if (effectiveKey) {
      ...
      for (const candidate of buildGitlabGroupIdentifierCandidates(body.baseUrl, effectiveKey)) {
      ...
      if (!resolvedGroupId) {
        const normalizedGroupKey = effectiveKey.replace(/^\/+|\/+$/g, '');
      ...
      if (!resolvedGroupId) {
        for (const candidate of buildGitlabProjectIdentifierCandidates(body.baseUrl, effectiveKey)) {
        ...
        if (!resolvedProjectId) {
          const normalizedProjectKey = effectiveKey.replace(/^\/+|\/+$/g, '');
