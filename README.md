...
이번 릴리스(`v0.8.0`)에서는 설정 흐름을 `프로젝트 관리 -> 그룹 관리 -> 스코어링 가중치 -> 계정 관리` 기준으로 재구성했고, 프로젝트 관리 안에서 멤버 조회/매핑/동기화까지 이어지도록 정리했습니다.  
또한 대시보드 탭 이동 로딩 UX, 프로젝트 필터, Gantt 색상, 초기 조회 성능을 개선하고, 프로젝트 제거 시 TeamScope 내부 스냅샷과 캐시만 정리하도록 데이터 경계를 강화했습니다.
...
`설정 > 계정 관리`에서 워크스페이스 접근을 관리할 수 있습니다. 계정 관리는 프로젝트/그룹/스코어링 같은 운영 설정과 분리되어 설정 탭의 마지막 영역에 배치됩니다.
...
| 개발자 | 개인별 점수와 비교 |
| 설정 | 프로젝트 연결, 프로젝트 기준 멤버 조회/매핑, 그룹, 가중치, 계정 관리 |
